...
**Relationship to REIXS:** This DDD provides canonical definitions for property management workflows including tenant relations, rent collection, maintenance, vendor oversight, budgeting, compliance, and performance reporting. Where concepts overlap with the core glossary, this DDD references `core_commercial_re_na@1.0.0` rather than redefining shared terms.

---

## Sections

The schema defines **10 sections** with **166 fields** total. All sections represent operational data used in day-to-day property management of commercial office and industrial assets.

### 1. tenantManagement

Tenant roster and relationship tracking.

Tenant identity fields use definitions from `core_commercial_re_na@1.0.0 → parties`.

| Field | Type | Description |
|---|---|---|
| `tenantName` | string | Full legal name of the tenant entity |
...
| `contactName` | string | Primary day-to-day contact person for the tenant |
...
Financial metrics use definitions from `core_commercial_re_na@1.0.0 → financialFundamentals`. Area metrics use definitions from `core_commercial_re_na@1.0.0 → areaAndMeasurement`.
...
| `rentableSF` | number | Rentable square footage per BOMA standards |
| `usableSF` | number | Usable square footage per BOMA standards |
| `tenantName` | string | Name of the current tenant, or "vacant" if unoccupied |
...
| `freeRentMonths` | number | Number of free rent months granted under the lease |
| `tiAllowance` | number | Tenant improvement allowance per rentable square foot |
...
Financial metrics use definitions from `core_commercial_re_na@1.0.0 → financialFundamentals`. Period definitions use `core_commercial_re_na@1.0.0 → dateAndTimePeriods`.

| Field | Type | Description |
|---|---|---|
| `budgetPeriod` | string | Budget period label (e.g., "FY 2025", "CY 2025") |
| `budgetType` | string | Period basis: fiscal year, calendar year |
| `revenueBaseRent` | number | Total budgeted or actual base rent revenue |
| `revenuePercentageRent` | number | Total budgeted or actual percentage rent revenue |
| `revenueRecoveries` | number | Total budgeted or actual tenant recovery revenue (CAM, taxes, insurance) |
| `revenueParking` | number | Total budgeted or actual parking revenue |
| `revenueOther` | number | Total budgeted or actual miscellaneous revenue (antenna, signage, storage) |
| `expenseUtilities` | number | Total budgeted or actual utility expense (electricity, gas, water, sewer) |
| `expenseRepairsMaintenance` | number | Total budgeted or actual repairs and maintenance expense |
...
| `expenseInsurance` | number | Total budgeted or actual property insurance expense |
...
Financial metrics use definitions from `core_commercial_re_na@1.0.0 → financialFundamentals`.
...
| `operatingExpenseRatio` | number | Operating expenses divided by gross revenue (expressed as decimal) |
