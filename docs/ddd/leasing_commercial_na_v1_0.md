...
**Relationship to REIXS:** This DDD defines leasing-specific fields for market intelligence, deal structuring, and transaction management. It references the core glossary (`re-ddd:core_commercial_re_na@1.0.0`) for shared property identification, financial fundamentals, party representation, and lease classification concepts rather than redefining them.

---

## Sections

The schema defines **10 sections** with **191 fields** total. Sections cover the full leasing lifecycle from market analysis through deal execution and tenant retention.
...
| Field | Type | Description |
|---|---|---|
| `prospectName` | string | Name of the prospective tenant entity |
...
| `tiAllowancePerSF` | number | Tenant improvement allowance offered (per SF) |
...
| `freeRentMonths` | number | Number of months of free rent offered |
...
| `efficiencyRatio` | number | Usable-to-rentable area ratio (expressed as decimal, e.g., 0.87). See also `re-ddd:core_commercial_re_na@1.0.0` areaAndMeasurement. |
...
| `concludedMarketRentPerSF` | number | Concluded market rent (per SF per annum) |
