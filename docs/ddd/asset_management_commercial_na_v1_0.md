...
**Relationship to REIXS:** This DDD provides canonical definitions for asset management workflows including valuation, financial performance, debt structuring, hold/sell analysis, leasing strategy, capital planning, risk assessment, and portfolio context. Where concepts overlap with the core glossary, this DDD references `core_commercial_re_na@1.0.0` rather than redefining shared terms.
...
| `acquisitionDate` | string (ISO 8601) | Date the asset was acquired |
...
| `revenueBaseRent` | number | Revenue from base rent collections |
| `revenueRecoveries` | number | Revenue from expense recoveries (CAM, taxes, insurance) |
| `revenueParking` | number | Revenue from parking operations |
| `revenueOther` | number | Revenue from other sources (storage, antenna, signage, etc.) |
...
Valuation concepts use definitions from `core_commercial_re_na@1.0.0 → financialFundamentals`.

| Field | Type | Description |
|---|---|---|
| `goingInCapRate` | number | Capitalization rate applied at acquisition (expressed as decimal) |
| `exitCapRate` | number | Assumed capitalization rate at disposition (expressed as decimal) |
...
| `markToMarketPercent` | number | Percentage change from book value to current market value |
...
| `environmentalRiskFloodZone` | string | FEMA flood zone designation (e.g., Zone X, Zone AE) |
...
Temporal concepts use definitions from `core_commercial_re_na@1.0.0 → dateAndTimePeriods`.
...
| `scenarioBaseCaseIRR` | number | Base case scenario projected IRR (expressed as decimal) |
| `scenarioUpsideNOI` | number | Upside scenario projected NOI |
| `scenarioUpsideIRR` | number | Upside scenario projected IRR (expressed as decimal) |
| `scenarioDownsideNOI` | number | Downside scenario projected NOI |
| `scenarioDownsideIRR` | number | Downside scenario projected IRR (expressed as decimal) |
